...
/****
 * fonction pour l'affichage des customs post type creation
 * 
 */
function mes_creations_affichage($query)
{
    if ($query->is_main_query() && !is_admin() && (is_post_type_archive('creation') || is_tax('categorie_creation'))) {
        $query->set('posts_per_page', 6); // Nombre de posts par page
        $query->set('orderby', 'date'); // Trier par date
        $query->set('order', 'DESC'); // Ordre décroissant
    }
};
add_action('pre_get_posts', 'mes_creations_affichage');
